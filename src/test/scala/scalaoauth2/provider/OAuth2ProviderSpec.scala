...
class OAuth2ProviderSpec extends AnyFlatSpec {
  ...
  object TestOAuthProvider extends OAuth2Provider {
    override def responseAccessToken[U](r: GrantHandlerResult[U]) =
      super.responseAccessToken(r) ++ Map(
        "custom_key" -> JsString("custom_value")
      )
  }
  ...
  it should "return error message as JSON" in {
    val json = TestOAuthProvider.responseOAuthErrorJson(
      new InvalidRequest("request is invalid")
    )
  ...
  it should "return error message to header" in {
    val (name, value) = TestOAuthProvider.responseOAuthErrorHeader(
      new InvalidRequest("request is invalid")
    )
    name should be("WWW-Authenticate")
    value should be(
      """Bearer error="invalid_request", error_description="request is invalid""""
    )
  }

  it should "get parameters from form url encoded body" in {
    val values = Map("id" -> List("1000"), "language" -> List("Scala"))
  ...
  it should "get parameters from JSON body" in {
    val json = Json.obj("id" -> 1000, "language" -> "Scala")
    val request = FakeRequest(
      method = "GET",
      uri = "/",
      headers = FakeHeaders(),
